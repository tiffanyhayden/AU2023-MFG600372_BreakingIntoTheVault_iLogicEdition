...
#End Region
    ...
    'Public Function Item_FindByPropName(Optional strItemPropName As String = "Number", Optional strItemPropValue As String = "", Optional intSrchOper As Integer = 3) As ACW.Item()
    '    Item_FindByPropName = Nothing
    ...
    '    Dim oItemService As ACW.ItemService = oConnection.WebServiceManager.ItemService
    '    Dim oFile As ACW.File = Nothing
    '    Dim oPropDefs As ACW.PropDef() = oConnection.WebServiceManager.PropertyService.GetPropertyDefinitionsByEntityClassId("ITEM")
    '    Dim oPropDef As ACW.PropDef = oPropDefs.[Single](Function(n) n.DispName = strItemPropName)
    '    Dim oSearch As New ACW.SrchCond()
    '    'Dim oFolder As ACW.Folder = Nothing
    '    'Dim oFolders As ACW.Folder() = Nothing
    '    Dim strBookmark As String = String.Empty
    ...
    '    Dim oItems As ACW.Item() = Nothing
    ...
    '        oSearch.SrchOper = intSrchOper
    '        oSearch.SrchRule = ACW.SearchRuleType.Must
    '        oSearch.SrchTxt = strItemPropValue
    '    Catch EX As Exception
    '        MsgBox("Search criteria failed.", MsgBoxStyle.OkOnly, "Error")
    '        Exit Function
    '    End Try
    ...
    '    '*******************************************************************************************
    '    Try
    '        oItems = oConnection.WebServiceManager.ItemService.FindItemRevisionsBySearchConditions(New ACW.SrchCond() {oSearch}, Nothing, True, strBookmark, oStatus)
    '    Catch EX As Exception
    '        MsgBox("Search Failed.", MsgBoxStyle.OkOnly, "Error")
    '        Exit Function
    '    End Try
    ...
    'Public Sub Item_SetLifeCycleState(Optional strItemPropName As String = "Number", Optional strItemPropValue As String = "")
    '    Dim oItemService As ACW.ItemService = oConnection.WebServiceManager.ItemService
    '    Dim oItem As ACW.Item
    '    Dim oItems As ACW.Item()
    '    Dim lgItemIDs As New List(Of Long)
    '    Dim lgToStateIds As New List(Of Long)
    '    Dim oLifecycleService As ACW.LifeCycleServiceBase = oConnection.WebServiceManager.LifeCycleService
    '    Dim oLfCycDefs As ACW.LfCycDef() = oLifecycleService.GetAllLifeCycleDefinitions()
    '    Dim oLfCycDef As ACW.LfCycDef
    '    Dim oLfCycState As ACW.LfCycState
    '    Dim lgToStateId As Long
    '    Dim strToLfCycState As String = "Released"

    '    For Each oLfCycDef In oLfCycDefs
    '        If oLfCycDef.DispName = "Item Release Process" Then
    '            For Each oLfCycState In oLfCycDef.StateArray
    '                If oLfCycState.DispName = strToLfCycState Then
    '                    lgToStateId = oLfCycState.Id
    '                End If
    '            Next
    '        End If
    '    Next

    '    oItems = Item_FindByPropName(strItemPropName, strItemPropValue)
    '    MsgBox("oItemCount: " & oItems.Count)


    '    For Each oItem In oItems

    '        lgItemIDs.Add(oItem.MasterId)
    '        lgToStateIds.Add(lgToStateId)
    '    Next
    '    MsgBox("lgItemIDs: " & lgItemIDs.Count)
    '    MsgBox("lgToStateIds: " & lgToStateIds.Count)

    '    oItemService.UpdateItemLifeCycleStates(lgItemIDs.ToArray, lgToStateIds.ToArray, "Moved To Released")
    ...
    'Public Function DidYouCreateTheLatestVersion(strFileName As String) As Boolean

    '    '*******************************************************************************************
    '    ' CHECK MAJOR REQUIREMENTS FOR THIS FUNCTION AND EXITS EARLY IF NEEDED. 
    '    '*******************************************************************************************
    '    DidYouCreateTheLatestVersion = False
    '    If strFileName = "" Then
    '        MsgBox("strFileName must have a value to continue", MsgBoxStyle.OkOnly, "Error")
    '        Exit Function
    '    End If
    ...
    '    '*******************************************************************************************
    '    ' FIND THE FIRST VESIONS CREATOR THAT IS NOT ADMIN 
    '    ' DOING THIS IGNORES PROPERTY UPDATES THAT ARE TYPICALLY ADMIN
    '    '*******************************************************************************************
    '    If oFileVersions IsNot Nothing Then
    '        For Each oFileVersion In oFileVersions
    '            If InStr(oFileVersion.CreateUserId.ToString.ToUpper, "ADMIN") > 0 Then
    '                'Skip
    '            Else
    '                strCreator = oFileVersion.CreateUserName.ToUpper
    '                Exit For
    '            End If
    '        Next
    '    End If
    ...
    '    '*******************************************************************************************
    '    ' COMPARE VERSION CREATOR WITH COMPUTER NAME TO SEE IF THEY ARE EQUAL
    '    '*******************************************************************************************
    '    If strCreator = strUserName Then DidYouCreateTheLatestVersion = True


    'End Function

    'Function IsOutOfDate(strFileName As String) As Boolean
    '    '*******************************************************************************************
    '    ' THIS FUNCTION CHECKS THE OUT OF DATE STATUS FOR A FILE
    '    '*******************************************************************************************
    ...
    '    '*******************************************************************************************
    '    ' OUT OF DATE STATUS STATUS FOR A FILE
    '    '*******************************************************************************************
    '    If oFile IsNot Nothing Then
    '        oFileIteration = New VDF.Vault.Currency.Entities.FileIteration(oConnection, oFile)
    '        oFileIteration.
    '        MsgBox("oFileIteration Version Number : " & oFileIteration.)
    '        Try
    '            IsOutOfDate = oFileIteration.IsOutOfDate
    '            MsgBox("IsOutOfDate: " & IsOutOfDate)
    '        Catch ex As Exception
    '            MsgBox("Out of date status was not available.", MsgBoxStyle.OkOnly, "Error")
    '            Exit Function
    '        End Try
    '    End If
    ...
    'Function IsLatestVersion(strFileName As String) As Boolean
    '    '*******************************************************************************************
    '    ' THIS FUNCTION RETURNS IF THE FILE LOCALLY IS THE LATEST VERSION
    '    '*******************************************************************************************
    ...
    '    '*******************************************************************************************
    '    ' CHECK OUT OF DATE STATUS FOR A FILE
    '    '*******************************************************************************************
    '    If oFile IsNot Nothing Then
    '        oFileIteration = New VDF.Vault.Currency.Entities.FileIteration(oConnection, oFile)
    '        Try
    '            IsLatestVersion = oFileIteration.IsLatestVersion
    '        Catch ex As Exception
    '            MsgBox("IsLatestVersion was not available.", MsgBoxStyle.OkOnly, "Error")
    '            Exit Function
    '        End Try
    '    End If
    ...
    'Function GetDateModified(strFileName As String) As Date
    '    '*******************************************************************************************
    '    ' THIS FUNCTION RETURNS IF THE FILE LOCALLY IS THE LATEST VERSION
    '    '*******************************************************************************************
    '    '*******************************************************************************************
    ...
    '        oFileIteration = New VDF.Vault.Currency.Entities.FileIteration(oConnection, oFile)
    '        Try
    '            GetDateModified = oFileIteration.LastModifiedDate

    '        Catch ex As Exception
    '            MsgBox("LastModifiedDate was not available.", MsgBoxStyle.OkOnly, "Error")
    '        End Try
    '    End If
    ...
    '#Region "Vault Addin"
    '    Public Function Vault_IsSignedIn() As Boolean
    '        Vault_IsSignedIn = False

    '        If oEDMS.IsSignedIn Or oEDMS.IsSignedInAsAnonymous Then
    '            Vault_IsSignedIn = True
    '        End If

    '    End Function

    '    Public Function Vault_IsLoaded() As Boolean
    '        Vault_IsLoaded = False

    '        If oEDMS.IsLoaded Then
    '            Vault_IsLoaded = True
    '        End If
